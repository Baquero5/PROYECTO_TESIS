...
async def get_subcategorias(
    ...
    if id_categoria:
        return await repo.get_by_categoria(id_categoria)
...
async def get_subcategoria(
    ...
    repo = SubcategoriaRepository(db)
    subcategoria = await repo.get_by_id(subcategoria_id)
    if not subcategoria:
...
async def create_subcategoria(
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=409, detail="Ya existe una subcategoria con ese nombre")
...
async def update_subcategoria(
    ...
    repo = SubcategoriaRepository(db)
    subcategoria = await repo.update(subcategoria_id, **data.model_dump(exclude_unset=True))
    if not subcategoria:
        raise HTTPException(status_code=404, detail="Subcategoria no encontrada")
    return subcategoria
